...
def preprocess_data(df, pattern=r'^S...C.........C'):
    """
    Preprocess the input DataFrame:
    - Filter peptides with length >= 12
    - Keep peptides starting with 'S'
    - Remove rows containing '*' or 'blank'
    - Trim peptides to the first 15 amino acids
    """
    df = df[df['peptide'].str.len() >= 15]
...
def merge_data(df1, df2):
    ...
    # Rename columns in df1 with input suffix
    ...
    df_merged = pd.merge(df1_renamed, df2_renamed, on='peptide', how='outer')
    df_merged.fillna(0, inplace=True)
    return df_merged

def process_afterDE(file_path, output_path):
    """
