...
import pandas as pd
import argparse
from data_processing import preprocess_data, aggregate_reads, merge_data, assign_group
...
# Step 2: Remove the header and extract data lines
    ...
    print("Skipping the first 3 columns...")
    df = df.iloc[:, 3:]
    print("Columns after skipping the first 3:", df.columns)

      # Step 5: Keep the last N columns (1, 2, or 3)
    num_columns_to_keep = min(len(df.columns), 6)  # Dynamically handle up to 3 columns
    ...
    new_column_names = ['SDB']+['DNA']+['peptide'] + [f'reads{i+1}' for i in range(num_columns_to_keep - 3)]
...
def cleaning(input_file1, input_file2, output_file, pattern=r'^S...C.........C', library="WL0.SDB21", baseline="WL0.SDB17"):
    # Load the data
    df1 = pd.read_csv(input_file1, index_col=False)
    df2 = pd.read_csv(input_file2, index_col=False)

    # Preprocess the data
    df1 = preprocess_data(df1, pattern=pattern, library=library, baseline=baseline)
    df2 = preprocess_data(df2, pattern=pattern, library=library, baseline=baseline)
