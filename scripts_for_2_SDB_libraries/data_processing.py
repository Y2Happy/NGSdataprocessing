...
def preprocess_data(df, pattern, library, baseline):
    ...
    df = df[df['peptide'].apply(lambda x: isinstance(x, str))]
...
def merge_data(df1, df2):
    ...
    for col in df1.columns:
        if col != 'peptide':
            df1_renamed = df1_renamed.rename(columns={col: f"{col}{input_suffix}"})
    
    # Rename columns in df2 with output suffix
    for col in df2.columns:
        if col != 'peptide':
            df2_renamed = df2_renamed.rename(columns={col: f"{col}{output_suffix}"})
    df_merged = pd.merge(df1_renamed, df2_renamed, on='peptide', how='outer', suffixes=('_input', '_output'))
    df_merged.fillna(0, inplace=True)
    return df_merged

# Assign groups based on patterns
def assign_group(sequence, library):
...
def process_afterDE(file_path, output_path, pattern=r'^S...C.........C', inputtiter=1e10, outputtiter=1e7):
    """
    ...
    df['group'] = df['sequence'].apply(lambda x: assign_group(x, pattern))
    
    df['inputtiter'] = (df['input_average'] / df['input_average'].sum()) * inputtiter
    df['outputtiter'] = (df['output_average'] / df['output_average'].sum()) * outputtiter
